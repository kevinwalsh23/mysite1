...
function addMarker(place)
{
   // var stuff = "http://maps.google.com/mapfiles/kml/pal3/icon40.png";
    ...
    id: place.bar_id
        ...
        $('#' + place.bar_id)[0].scrollIntoView(true);
...
$(document).ready(function () {
    $(".resizable").click(function() {
        if (mapclick == '0') {
            ...
            $(".right-content-area").css("height", "55%");
            $(".right-content-area").css("top", "45%");
            //console.log("WORKS!");
            mapclick = 1;
            //console.log(mapclick);
            return;
        ...
        $(".right-content-area").css("height", "15%");
        $(".right-content-area").css("top", "85%");
            //console.log("WORKS!");
            //mapclick = 0;
...
$(document).ready(
    function(){
        $('.rating').each(function(index) {
            //var rating = $('#seconds')[0].innerHTML;
            var rating = $('.rating')[index].innerHTML;
            //console.log(rating);
            //console.log(max);
            //console.log(min);
            if ((rating) >= 9) {
                //console.log(max - min);
                //$('.mininterval').eq(index).html("Ending in " + (max-min) +" minutes !");
                $('.rating').eq(index).css("background-color", "green");
                //console.log("WHAT");
                //$('.mininterval').eq(index).show();
                ...
                $('.rating').eq(index).css("background-color", "SeaGreen");
                //$('.mininterval').eq(index).show();
                ...
                //console.log(max - min);
                //$('.mininterval').eq(index).html("Ending in less than two hours!");
                $('.rating').eq(index).css("background-color", "MediumSeaGreen");
                //$('.mininterval').eq(index).show();
                ...
                //console.log(max - min);
                //$('.mininterval').eq(index).html("Ending in less than two hours!");
                $('.rating').eq(index).css("background-color", "GoldenRod");
                //$('.mininterval').eq(index).show();
